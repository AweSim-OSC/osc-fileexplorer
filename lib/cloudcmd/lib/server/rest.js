(function() {
    ...
    var DIR         = './',
        ...
        Rsync       = require('rsync'),
        RsyncMitter = require(DIR + 'rsyncmitter'),
    ...
    function rsyncVersion() {
      ...
      if((!! match) === false || match.length < 2) {
        return null;
      }

      return match[1].split('.').map(function(e) {return parseInt(e)});
    }

    /**
     * Test if Rsync's --info=progress2 can be used by checking the version
     * 3.1.0 is the version that first provided --info=progress2
     * https://download.samba.org/pub/rsync/src/rsync-3.1.0-NEWS
     * @return {boolean}
     */
    function canUseRsyncInfo() {
      const version = rsyncVersion();

      return !! (
        version &&              // version is not null?
        version.length === 3 && // version parsed into major, minor, patch?
        version[0] >= 3 &&      // major version at least 3?
        version[1] >= 1         // minor version at least 1?
      );
    }
    ...
    function copy(from, to, names, fn) {
        var error,
            ...
            rsync_cmd = new Rsync()
                .archive()  // preserves permissions, handles directories, etc
                .source(names.map(x => from + x))
                .destination(to);

            // The side effect of not setting the progress flag is that no
            // progress messages will be sent to the user
            if(canUseRsyncInfo() && false) {
                // needed to get total transfer progress
                rsync_cmd.set('info', 'progress2');
            }

        var rsyncer = new RsyncMitter(rsync_cmd);

        rsyncer.on('error', function(e) {
            error = e;
        });

        rsyncer.on('progress', function(count) {
