(function() {
    ...
    var DIR         = './',
        ...
        Rsync       = rewire('rsync'),
        exposeLongOption = Rsync.__get__('exposeLongOption'),
    ...
    // Allow Rsync to use --info
    exposeLongOption('info');
    ...
    function copy(from, to, names, fn) {
        var error,
            ...
            rsync_cmd = new Rsync()
                .archive()
                .info('progress2')  // needed to get total transfer progress
                .progress()
                .source(names.map(x => from + x))
                .destination(to);

        let last_progress = 0;
        let app_process = process;
        rsync_cmd.execute(
            // on end
            function(e, code, cmd) {
                if(e === null) {
                    app_process.stdout.write('\n');
                    fn(error);
                } else {
                    error = e;
                    // TODO: Copymitter has an abort method. Look into what it does, and if we need to add that here.
                }
            // on 'progress'
            }, function(data){
                let data_string = data.toString();
                if(mayHavePercentComplete(data_string)) {
                    let new_progress = percentComplete(data_string);
                    // Sometimes rsync's progress goes down; let's only go up
                    if(new_progress > last_progress) {
                        last_progress = new_progress;
                        app_process.stdout.write(rendy(tmpl, {
                            to: to,
                            from: from,
                            count: last_progress
                        }));
            ...
            // on 'progress' with error
            }, function(data) {
                console.error('stderr generated by rsync command(' + rsync_cmd.command() + '): ' + data.toString());
            }
        );
